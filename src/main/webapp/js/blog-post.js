...
$(document).ready(function(){
	...
	$('#postshrf').click(function() {
		...
		if(typeof(cookieUserName) !== "undefined" && cookieUserName !== null){
			$.ajax({
				url : 'http://localhost:8080/stack-flood/online/posts/',
				type : 'get',
					...
					if(typeof(response) !== "undefined" && response !== null){
						...
						if(response.length > 0){
							...
							posts.forEach(function(post) {
								  ...
								  anchorDt.trim();
								  tr.append('<div id="data-'+i+'"><div id="view-'+i+'"><div id="viewc-'+i+'">'+post.viewCount+'</div><div id="viewt-'+i+'">views</div></div><div id="reply-'+i+'"><div id="replyc-'+i+'">'+post.replyCount+'</div><div id="replyt-'+i+'">replies</div></div><div id="content-'+i+'"><div id="header-'+i+'">'+post.title+'<span style="display:none">'+post.postId+'</span></div><div id="footer-'+i+'"><div id="tagd-'+i+'">'+anchorDt+'</div><div id="footerright-'+i+'"><div id="date-'+i+'">'+new Date(post.createdDate).toString().split('G')[0].trim()+'</div><div id="untdet-'+i+'">'+cookieUserName+'</div></div></div></div></div>');
								  table.append(tr);
								  i=i+1;
							});
